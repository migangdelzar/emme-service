...
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcPaymentWorkflowCheckpointRepository.java` | JdbcClient survivor: atomic transition | `ai_workflow_run` payment/lifecycle status transitions | Expected-status UPDATE claim, version increment, and terminal/retry state recording remain atomic | Tenant schema checkout plus principal/conversation/workflow ownership; no repeated tenant predicate in schema-local query | `PaymentWorkflowCheckpointRepository` / `JdbcPaymentWorkflowCheckpointRepository` | `JdbcPaymentWorkflowCheckpointRepositoryTest`, payment resume tests; live PostgreSQL concurrency gate |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcPaymentWorkflowExecutionContextRepository.java` | JdbcClient survivor: LangGraph checkpoint | `ai_workflow_run` backend principal/conversation/idempotency context | Single scalar lookup before callback resume; no entity lifecycle | Tenant schema checkout and RLS are authoritative; workflow ID is explicit ownership key | `PaymentWorkflowExecutionContextRepository` / `JdbcPaymentWorkflowExecutionContextRepository` | `PaymentWorkflowEventListenerTest`, live PostgreSQL resume gate |
...
|---|---|---|
| `libraries/ai-contracts/src/main/java/com/emme/ai/contracts/model/AiModelProvider.java` | Deleted | Composite contract removed in `568241c5` on 2026-09-05 after repository-wide production caller inventory was clean; canonical chat, embedding, image, and retrieval capabilities are independently composed |
| `modules/ai-platform/src/main/java/com/emme/ai/platform/adapter/out/provider/springai/SpringAiModelProvider.java` | Deleted | Composite Spring provider removed in `568241c5` on 2026-09-05 after all capability callers migrated and configuration/integration tests were moved to canonical ports |
