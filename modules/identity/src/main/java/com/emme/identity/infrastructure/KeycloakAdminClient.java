...
public class KeycloakAdminClient {
    ...
    public void createRealm(String realmName, String displayName) throws IOException {
        ...
        var req = new Request.Builder()
            ...
            .post(RequestBody.create(json.writeValueAsString(body),
                MediaType.get("application/json")))
            .build();
        try (var resp = http.newCall(req).execute()) {
            if (resp.code() == 409) return; // realm already exists
            if (resp.code() != 201)
                throw new IOException("Realm create failed: HTTP " + resp.code());
        }
    }
    ...
    public void createClient(String realm, String clientId, List<String> redirectUris)
        ...
        try (var resp = http.newCall(req).execute()) {
            ...
            if (resp.code() != 201)
                throw new IOException("Client create failed: HTTP " + resp.code());
        }
    }
    ...
    public void createRealmRole(String realm, String roleName) throws IOException {
        ...
        try (var resp = http.newCall(req).execute()) {
            if (resp.code() == 409) return;
            if (resp.code() != 201)
                throw new IOException("Role create failed: HTTP " + resp.code());
        }
    }
    ...
    public String createUser(String realm, String username, String email,
                             String password, String roleName) throws IOException {
        var token = getAdminToken();
        String userId;
        ...
        var userReq = new Request.Builder()
            .url(baseUrl + "/admin/realms/" + realm + "/users")
            .header("Authorization", "Bearer " + token)
            .header("Content-Type", "application/json")
            .post(RequestBody.create(json.writeValueAsString(userBody),
                MediaType.get("application/json")))
            .build();
        try (var resp = http.newCall(userReq).execute()) {
            if (resp.code() == 409) {
                // User already exists — look up their ID
                var searchReq = new Request.Builder()
        ...
        var pwdReq = new Request.Builder()
            .url(baseUrl + "/admin/realms/" + realm + "/users/" + userId + "/reset-password")
            .header("Authorization", "Bearer " + token)
            .header("Content-Type", "application/json")
            .put(RequestBody.create(json.writeValueAsString(pwdBody),
                MediaType.get("application/json")))
            .build();
        try (var resp = http.newCall(pwdReq).execute()) {
            if (resp.code() != 204)
                throw new IOException("Password set failed: HTTP " + resp.code());
        }

        // Step 3: Look up role ID
        var roleReq = new Request.Builder()
        ...
        try (var resp = http.newCall(roleReq).execute()) {
            if (!resp.isSuccessful())
                throw new IOException("Role lookup failed: HTTP " + resp.code());
            var node = json.readTree(resp.body().string());
        ...
        var assignReq = new Request.Builder()
            .url(baseUrl + "/admin/realms/" + realm + "/users/" + userId +
                "/role-mappings/realm")
            .header("Authorization", "Bearer " + token)
            .header("Content-Type", "application/json")
            .post(RequestBody.create(json.writeValueAsString(assignBody),
                MediaType.get("application/json")))
            .build();
        try (var resp = http.newCall(assignReq).execute()) {
            if (resp.code() != 204)
                throw new IOException("Role assignment failed: HTTP " + resp.code());
        }
        return userId;
    }
}
