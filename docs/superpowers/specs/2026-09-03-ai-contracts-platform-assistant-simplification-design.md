...
| `src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcDesignImageMetadataRepository.java` | Keep `JdbcClient` | Two small metadata statements do not justify JPA entity/repository indirection; the port remains provider-neutral |
| `src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcConversationWorkflowReviewAuditAdapter.java` | Keep `JdbcClient` | One append-only JSONB insert is shorter and clearer as parameterized SQL; the port remains provider-neutral |
