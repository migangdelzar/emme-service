...
/**
 * Handles customer authentication via social login tokens from the
 * emme-customers Keycloak realm. Creates or updates global customer
 * identity. Does NOT handle staff or platform login.
 */
@Service
public class CustomerAuthService {
    ...
    public CustomerAuthService(CustomerIdentityRepository customerRepo,
                               MultiRealmJwtDecoder jwtDecoder) {
        this.customerRepo = customerRepo;
        this.jwtDecoder = jwtDecoder;
    }

    /**
     * Validates a provider token from the emme-customers realm and
     * returns or creates the customer identity record.
     */
    @Transactional
    public CustomerLoginResult authenticate(String providerToken) {
        ...
        CustomerIdentity customer = customerRepo
        ...
        if (name != null && !name.equals(customer.getName())) {
            customer.setName(name); changed = true;
        }
        if (avatarUrl != null && !avatarUrl.equals(customer.getAvatarUrl())) {
            customer.setAvatarUrl(avatarUrl); changed = true;
        }
