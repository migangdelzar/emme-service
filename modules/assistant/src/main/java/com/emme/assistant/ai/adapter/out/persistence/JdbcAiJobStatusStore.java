...
public final class JdbcAiJobStatusStore implements AiJobStatusStore {
  ...
  public void fail(UUID jobId, String errorCode, AiExecutionContext context) {
    jdbc.update(
        "UPDATE emme_core.ai_job_state SET status=CASE WHEN attempts >= ? THEN 'DEAD_LETTER' ELSE 'RETRYING' END, available_at=CURRENT_TIMESTAMP + ((2 ^ GREATEST(attempts-1,0)) * INTERVAL '1 second'), last_error=?, updated_at=CURRENT_TIMESTAMP WHERE job_id=? AND tenant_id=? AND status='CLAIMED'",
        maxAttempts,
        errorCode,
        jobId,
        context.tenantId());
