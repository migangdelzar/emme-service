...
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcSemanticReferenceSearchAdapter.java` | Keep | Rename to `PostgresSemanticReferenceRetriever`; pgvector reference search is specialized |
...
|---|---|---|---|---|---|---|
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/graph/JdbcAgeGraphClient.java` | Lower-level boundary: AGE | AGE graph nodes/edges plus `ai_age_graph_registry` projection metadata | AGE extension load, graph creation, Cypher `MERGE`, and registry upsert execute in one projection transaction | Graph name and all node/edge predicates are tenant-scoped; AGE search path is local to the transaction | `PostgresAgeGraphStore` | `AgeGraphIntegrationTest` |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcAiJobStatusStore.java` | JdbcClient survivor: atomic claim | `ai_job_state` lifecycle rows and JSON/text job payload | Conditional `UPDATE ... RETURNING`, lease recovery, and `FOR UPDATE SKIP LOCKED` must remain one atomic claim transaction | Tenant predicate plus `current_tenant_id()` and database RLS protect every state transition | `PostgresAiJobStateStore` | `JdbcAiJobStatusStoreTest`, `AiJobReconciliationClaimIntegrationTest` |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcAiToolIdempotencyStore.java` | JdbcClient survivor: atomic idempotency, JSONB | `ai_tool_idempotency` status, lease, and JSONB result payload | Unique-key insert/expired-lease reclaim, completion, and release are conditional transitions | Tenant and principal are part of every key and predicate; RLS remains authoritative | `PostgresAiToolIdempotencyStore` | `JdbcAiToolIdempotencyStoreTest`, `QuoteWorkflowIdempotencyIntegrationTest` |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcAiTraceRecorder.java` | JPA candidate: JSONB | `ai_model_execution`, `ai_tool_call`, and `ai_semantic_execution` trace records with redacted JSONB | Durable upserts are not worker claims; JPA `@Version` is sufficient if the JSONB converter is small and tested | Active execution context and database RLS provide tenant/principal isolation | `JpaAiTraceRecorder` with module-private trace entities | `JdbcAiTraceRecorderTest` plus a JPA JSONB slice test |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcConversationWorkflowReviewAuditAdapter.java` | JPA candidate: JSONB | Append-only `ai_conversation_workflow_review_decision` audit row with clarification JSONB | Single append operation; no claim invariant | Explicit tenant/workflow/conversation/reviewer identity match plus RLS | `JpaConversationWorkflowReviewAuditAdapter` | `JdbcConversationWorkflowReviewAuditAdapterTest` |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcDesignImageMetadataRepository.java` | JPA candidate | `ai_design_image` metadata rows linked to a workflow | Ordinary insert/delete CRUD; no lock or provider-specific SQL | Delete includes tenant/workflow/storage key; save must retain RLS/context validation during migration | `JpaDesignImageMetadataAdapter` | `JdbcDesignImageMetadataRepositoryTest` plus a JPA mapping test |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcQuoteArtifactRepository.java` | JPA candidate: JSONB | `ai_extraction_result`, `quote_draft`, and `quote_review_task` aggregates with JSONB attributes | Unique tenant/workflow upserts; use JPA versioning only after FK/order and JSONB mapping are proven | Tenant is resolved from the active AI context and every record is RLS-protected | `JpaQuoteArtifactPersistenceAdapter` | `JdbcQuotePersistenceAdapterTest` plus quote JPA mapping/concurrency tests |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcQuoteReviewRepository.java` | JPA candidate: optimistic locking | `quote_review_task` plus append-only `quote_review_decision` rows | Conditional reviewer update on task version must map to `@Version` or an explicit conditional repository update | Tenant and reviewer identity are validated before resolve/decision writes | `JpaQuoteReviewPersistenceAdapter` | `JdbcQuotePersistenceAdapterTest` plus reviewer ownership/version tests |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcQuoteWorkflowRepository.java` | JPA candidate: optimistic locking | `ai_workflow_run` lifecycle and JSONB state | Unique idempotency key and expected-version update must remain atomic; JPA `@Version` is suitable if conflict behavior is preserved | Tenant, principal, and conversation predicates remain mandatory | `JpaQuoteWorkflowPersistenceAdapter` | `JdbcQuoteWorkflowRepositoryTest` plus workflow idempotency/version tests |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcSemanticCacheAdapter.java` | JdbcClient survivor: pgvector/JSONB | Expiring `ai_semantic_cache` vector rows, response JSONB, and hit counters | Vector nearest-neighbor query, idempotent upsert, and hit increment are database-native operations | Tenant/principal/context identity, model dimensions, expiry, and RLS are part of the query contract | `PostgresSemanticCacheStore` | `JdbcSemanticCacheAdapterTest`, `PgVectorSemanticIntegrationTest` |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcSemanticReferenceSearchAdapter.java` | JdbcClient survivor: pgvector/FTS/RRF family | Tenant reference embeddings from `ai_intent_reference` and `ai_tool_reference` | Vector distance, dimension/model predicates, and authorized-tool allowlist are clearer as parameterized SQL | Tenant and authorized tool keys are enforced in the query; table/column names come only from constants | `PostgresSemanticReferenceRetriever` | `JdbcSemanticReferenceSearchAdapterTest`, `PgVectorSemanticIntegrationTest` |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/workflow/JdbcLangGraphCheckpointSaver.java` | JdbcClient survivor: LangGraph checkpoint, JSONB | `ai_workflow_checkpoint` snapshots and `ai_workflow_run` JSONB state | LangGraph saver contract, checkpoint upsert, and workflow state update must stay compatible and ordered | Tenant, conversation, principal/staff reviewer access checks are explicit and RLS-backed | `PostgresLangGraphCheckpointStore` | `JdbcLangGraphCheckpointSaverTest`, `ConversationWorkflowCheckpointIntegrationTest` |
| `modules/ai-platform/src/main/java/com/emme/ai/platform/learning/JdbcLearningCandidateStore.java` | JdbcClient survivor: JSONB/idempotent insert | `ai_learning_candidate` candidate record with JSONB evidence | Unique fingerprint/model upsert returns the existing identity without a read-then-write race | Tenant and principal are bound to the AI execution context and RLS | `PostgresLearningCandidateStore` | `JdbcLearningCandidateStoreTest` |
| `modules/ai-platform/src/main/java/com/emme/ai/platform/learning/JdbcLearningCandidateStateStore.java` | JdbcClient survivor: atomic claim | Versioned status transition on `ai_learning_candidate` | Expected-version conditional update is one atomic state transition; a JPA read-modify-write would widen the race window | Candidate identity is checked against the bound tenant context before transition | `PostgresLearningCandidateStateStore` | `JdbcLearningCandidateStateStoreTest` |
| `modules/ai-platform/src/main/java/com/emme/ai/platform/learning/JdbcLearningCandidateEvaluationStore.java` | JdbcClient survivor: JSONB/idempotent insert | `ai_learning_candidate_evaluation` versioned evidence and metrics JSONB | FK-guarded unique evaluation upsert avoids duplicate evaluation rows | Tenant is supplied from the bound context and protected by RLS | `PostgresLearningCandidateEvaluationStore` | `JdbcLearningCandidateEvaluationStoreTest` |
