...
import com.emme.buildlogic.provider.deployment.KubernetesProvider
import com.emme.buildlogic.task.deployment.DeployTask
import com.emme.buildlogic.task.deployment.DeploymentStatusTask
...
class EmmeDeploymentPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        with(project) {
            ...
            val providerClass = when (target.lowercase()) {
            ...
            tasks.register("deployUp", DeployTask::class.java) {
                group = "deployment"; description = "Start deployment"
                action.set("up"); deploymentProvider.set(deployment)
            }
            tasks.register("deployDown", DeployTask::class.java) {
                group = "deployment"; description = "Stop deployment"
                action.set("down"); deploymentProvider.set(deployment)
            }
            tasks.register("deployApply", DeployTask::class.java) {
                group = "deployment"; description = "Apply deployment changes"
                action.set("apply"); deploymentProvider.set(deployment)
            }
            tasks.register("deployStatus", DeploymentStatusTask::class.java) {
                group = "deployment"; description = "Check deployment status"
                deploymentProvider.set(deployment)
            }
            tasks.register("deployLogs", Exec::class.java) {
                group = "deployment"; description = "Show deployment logs"
                executable = "echo"
                doFirst { val logs = deployment.get().logs(tail = 50); logger.lifecycle(logs) }
            }
        }
    }
}
