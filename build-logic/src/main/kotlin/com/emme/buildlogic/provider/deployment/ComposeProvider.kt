...
abstract class ComposeProvider : DeploymentProvider() {
    ...
    private fun composeFile(): File =
        parameters.deploymentDir.file("compose/compose.${parameters.profile.get()}.yml").get().asFile

    private fun envFile(): File =
        parameters.deploymentDir.file("compose/env/${parameters.profile.get()}.env").get().asFile
    ...
    private fun execute(executable: String, args: List<String>): DeployResult = try {
    ...
    private fun executeCommand(executable: String, args: List<String>): String {
