...
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcConversationWorkflowReviewAuditAdapter.java` | JPA candidate: JSONB | Append-only `ai_conversation_workflow_review_decision` audit row with clarification JSONB | Single append operation; no claim invariant | Explicit tenant/workflow/conversation/reviewer identity match plus RLS | `ConversationWorkflowReviewAuditPort` / module-private JPA adapter after mapping tests | `JdbcConversationWorkflowReviewAuditAdapterTest` |
