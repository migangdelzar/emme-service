...
/**
 * Multi-user pool for E2E REST tests. Logs in users on-demand — only when {@link #acquire()} or
 * {@link #acquire(Role, String)} needs a role/tenant combination that isn't already in the pool.
  ...
  private TestUser login(PoolUser pu) {
    try {
      ...
      if (response.statusCode() != 200) {
        System.err.printf(
            "[E2eUserPool] Login failed for %s: HTTP %d%n", pu.email, response.statusCode());
        return null;
      }
