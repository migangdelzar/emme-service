...
class AppointmentApiTest {
    ...
    @Test void shouldListAppointments() {
        var result = api.get("/api/v1/appointments", 403);
        assertThat(result).isNotNull();
    }
    @Test void shouldCreateAppointment() {
        var body = "{\"customerId\":\"" + UUID.randomUUID() + "\",\"serviceId\":\"" + UUID.randomUUID() + "\",\"startAt\":\"2026-12-01T10:00:00Z\"}";
        var result = api.post("/api/v1/appointments", body, 400);
        assertThat(result).isNotNull();
    }
    @Test void shouldRejectInvalidAppointment() {
