...
class SharedOwnershipConventionTest {
  ...
  void documentsTechnicalCapabilityOwnershipWithoutBusinessLayers() {
    ...
    assertThat(Files.exists(root.resolve("search/postgres/PostgresHybridSearch.java"))).isTrue();
  ...
  void everyEmbeddingMutationAndMaintenanceQueryRequiresTenantScope() throws Exception {
    ...
    assertThat(HybridSearch.class).isInterface();
    assertThat(Files.readString(contract)).doesNotContain("JdbcClient");
    String contents = Files.readString(postgresAdapter);
