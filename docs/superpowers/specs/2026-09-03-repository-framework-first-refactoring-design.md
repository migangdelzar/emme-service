...
| `libraries/testing` | `BaseWebTest.java`, `TestSecurityConfig.java`, generic fixture dependencies | Keep only generic web, security, repository, and container helpers. Tenant-aware full-context setup belongs to tenancy fixtures; provider fakes belong to their owning modules. |
| `modules/tenancy` test fixtures | `BaseTenantModuleTest.java`, `TenantWebTest.java`, `TestBootstrapJdbcConfig.java` | Own tenant provisioning helpers and the H2-safe bootstrap migration boundary. |
| `modules/identity` test fixtures | `MockIdentityProviderAdministrationConfig.java` | Provide a no-op `IdentityProviderAdministrationPort` fake without subclassing the Keycloak adapter. |
...
| `modules/assistant/.../AiHttpClient.java` | Deleted for WhatsApp transport; retain raw HTTP only for an explicitly documented unsupported provider protocol | Do not maintain a generic raw HTTP wrapper beside Spring AI or a provider-scoped Spring client without a capability gap record. |
