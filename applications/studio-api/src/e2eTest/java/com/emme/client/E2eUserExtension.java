...
/**
 * JUnit 5 extension that acquires an E2E test user from the pool,
 * creates a {@link UserSession}, and injects it into test method parameters.
 *
 * <p>Resolution order: {@code @WithUser} on method → class level.
 * Default if absent: PLATFORM_ADMIN, any tenant, PER_METHOD.
 */
public class E2eUserExtension implements
    ...
    private void acquire(ExtensionContext ctx, WithUser ann) {
        var user = ann.tenant().isEmpty()
            ? POOL.acquire(ann.role())
            : POOL.acquire(ann.role(), ann.tenant());
    ...
    private static WithUser resolve(ExtensionContext ctx) {
        ...
        throw new IllegalStateException(
            "@WithUser required on method or class for E2eUserExtension");
    }

    private static ExtensionContext.Store getStore(ExtensionContext ctx) {
        return ctx.getStore(ExtensionContext.Namespace.create(E2eUserExtension.class, ctx.getUniqueId()));
    }
}
