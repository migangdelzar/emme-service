...
|---|---|---|---|
| `modules/shared` | `persistence/jdbc/JdbcConnectionExecutor.java`, `search/HybridSearch.java`, shared test fixtures and `build.gradle.kts` | Keep one low-level connection boundary for Liquibase/bootstrap; keep `HybridSearch` as PostgreSQL-specific until equivalent vector/FTS/RRF support exists; reduce shared dependencies and test-fixture coupling | P0/P1 |
