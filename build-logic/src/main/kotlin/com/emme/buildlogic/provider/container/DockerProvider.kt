...
abstract class DockerProvider : ContainerRuntimeProvider() {
    ...
    override fun build(image: String, context: File, tags: List<String>): BuildResult {
        ...
        val imageId = output.lines().lastOrNull { it.startsWith("Successfully built") }
            ?.substringAfter("Successfully built ")?.trim() ?: "unknown"
    ...
    override fun push(image: String, registry: String): PushResult {
        val fullImage = if (registry.isNotBlank()) "$registry/$image" else image
        val output = executeCommand(listOf("push", fullImage))
        val manifest = output.lines().lastOrNull { it.contains("digest:") }
            ?.substringAfter("digest:")?.trim() ?: "unknown"
        return PushResult(manifest = manifest)
    }

    override fun scan(image: String, severity: String, output: File): ScanResult {
        output.parentFile.mkdirs()
        val execOutput = executeCommand(listOf("run", "--rm", "-v",
            "/var/run/docker.sock:/var/run/docker.sock",
            "aquasec/trivy", "image", "--severity", severity, "--format", "sarif",
            "--output", "/tmp/scan.sarif", image))
    ...
    private fun executeCommand(args: List<String>): String {
        ...
        val process = ProcessBuilder(listOf(parameters.executable.get()) + args)
