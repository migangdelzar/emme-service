...
public final class JdbcAiJobStatusStore implements AiJobStatusStore {
  ...
  public JdbcAiJobStatusStore(
    ...
    this(jdbc, properties.maxAttempts(), transactions);
  }
  ...
  public AiJobStatus claim(UUID jobId, AiExecutionContext context) {
    return inTenantTransaction(
        ...
        () -> {
          var rows =
              jdbc.queryForList(
                  "UPDATE emme_core.ai_job_state SET status='CLAIMED', attempts=attempts+1, updated_at=CURRENT_TIMESTAMP WHERE job_id=? AND tenant_id=? AND tenant_id=current_tenant_id() AND status IN ('QUEUED','RETRYING') AND available_at<=CURRENT_TIMESTAMP RETURNING status",
                  jobId,
                  context.tenantId());
  ...
  public void fail(UUID jobId, String errorCode, AiExecutionContext context) {
    inTenantTransaction(
        ...
        () -> {
          jdbc.update(
              "UPDATE emme_core.ai_job_state SET status=CASE WHEN attempts >= ? THEN 'DEAD_LETTER' ELSE 'RETRYING' END, available_at=CURRENT_TIMESTAMP + (power(2, GREATEST(attempts-1,0)) * INTERVAL '1 second'), last_error=?, updated_at=CURRENT_TIMESTAMP WHERE job_id=? AND tenant_id=? AND tenant_id=current_tenant_id() AND status='CLAIMED'",
              maxAttempts,
              errorCode,
              jobId,
              context.tenantId());
