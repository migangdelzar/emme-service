...
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcDesignImageMetadataRepository.java` | JPA candidate | `ai_design_image` metadata rows linked to a workflow | Ordinary insert/delete CRUD; no lock or provider-specific SQL | Delete includes tenant/workflow/storage key; save must retain RLS/context validation during migration | `DesignImageMetadataRepository` / module-private JPA adapter after mapping tests | `JdbcDesignImageMetadataRepositoryTest` plus a JPA mapping test |
