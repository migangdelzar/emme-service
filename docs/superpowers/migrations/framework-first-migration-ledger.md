...
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JpaConversationWorkflowReviewAuditAdapter.java` | Replacement tested | Stable append-only review audit persisted through Spring Data with a tested JSONB mapping |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JpaDesignImageMetadataRepository.java` | Replacement tested | Stable JPA metadata CRUD; binary content remains in external storage |
...
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcAiTraceRecorder.java` | JPA candidate: JSONB | `ai_model_execution`, `ai_tool_call`, and `ai_semantic_execution` trace records with redacted JSONB | Durable upserts are not worker claims; JPA `@Version` is sufficient if the JSONB converter is small and tested | Active execution context and database RLS provide tenant/principal isolation | `AiTraceRecorder` / module-private JPA adapter after mapping tests | `JdbcAiTraceRecorderTest` plus a JPA JSONB slice test |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JpaConversationWorkflowReviewAuditAdapter.java` | JPA replacement tested: JSONB | Append-only `ai_conversation_workflow_review_decision` audit row with clarification JSONB | Single append operation; no claim invariant | Explicit tenant/workflow/conversation/reviewer identity match plus RLS | `ConversationWorkflowReviewAuditPort` / `JpaConversationWorkflowReviewAuditAdapter` | `JpaConversationWorkflowReviewAuditAdapterTest` |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcQuoteArtifactRepository.java` | JPA candidate: JSONB | `ai_extraction_result`, `quote_draft`, and `quote_review_task` aggregates with JSONB attributes | Unique tenant/workflow upserts; use JPA versioning only after FK/order and JSONB mapping are proven | Tenant is resolved from the active AI context and every record is RLS-protected | `QuoteArtifactRepository` / module-private JPA adapter after mapping tests | `JdbcQuotePersistenceAdapterTest` plus quote JPA mapping/concurrency tests |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcQuoteReviewRepository.java` | JPA candidate: optimistic locking | `quote_review_task` plus append-only `quote_review_decision` rows | Conditional reviewer update on task version must map to `@Version` or an explicit conditional repository update | Tenant and reviewer identity are validated before resolve/decision writes | `QuoteReviewRepository` / module-private JPA adapter after mapping tests | `JdbcQuotePersistenceAdapterTest` plus reviewer ownership/version tests |
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JdbcQuoteWorkflowRepository.java` | JPA candidate: optimistic locking | `ai_workflow_run` lifecycle and JSONB state | Unique idempotency key and expected-version update must remain atomic; JPA `@Version` is suitable if conflict behavior is preserved | Tenant, principal, and conversation predicates remain mandatory | `QuoteWorkflowRepository` / module-private JPA adapter after mapping tests | `JdbcQuoteWorkflowRepositoryTest` plus workflow idempotency/version tests |
...
| `modules/assistant/src/main/java/com/emme/assistant/ai/adapter/out/persistence/JpaDesignImageMetadataRepository.java` | JPA replacement tested | Stable `ai_design_image` metadata CRUD; binary content remains external | Ordinary insert/delete with database unique `(tenant_id, workflow_id)` constraint; no claim or vector invariant | Explicit tenant/workflow/storage-key predicates remain in the Spring Data delete method; RLS remains database authority | `DesignImageMetadataRepository` / `JpaDesignImageMetadataRepository` | `JpaDesignImageMetadataRepositoryTest`, `AiDesignImageMigrationContractTest` |
