...
public class GoogleCalendarClient implements GoogleCalendarPort {
   ...
   */
  public List<CalendarBusyTimeRange> freeBusy(String calendarId, String timeMin, String timeMax) {
    if (!configured) {
  ...
  private List<CalendarBusyTimeRange> parseBusyTimes(String responseBody, String calendarId)
      throws Exception {
    ...
    List<CalendarBusyTimeRange> ranges = new ArrayList<>();
    for (JsonNode slot : busy) {
      ...
      ranges.add(new CalendarBusyTimeRange(start, end));
    }
    return ranges;
  }
}
