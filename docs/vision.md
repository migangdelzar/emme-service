...
- Spring Modulith's JDBC publication registry for durable internal event
  delivery, with Kafka retained as a deferred capability for approved external
  consumers.
...
- RabbitMQ and AMQP-based externalization; Kafka remains the selected future
  event-streaming transport when an external delivery boundary is approved.
